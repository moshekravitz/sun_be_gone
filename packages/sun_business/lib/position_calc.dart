...
class cTime
{
  ...
  cTime(int y, int m, int d, double h , double mi,double s){iYear = y; iMonth = m;iDay = d;dHours = h;dMinutes = mi;dSeconds = s;}
}
...
class Point {
 late double longitude;
  ...
  void setLong(double myLong) {longitude = myLong;}
  void setLat(double myLat) {latitude = myLat;}
  Point(this.longitude,this.latitude){}//(double myLong,double myLat) //{setLong(myLong);setLat(myLat);}
  Point.copy(Point myP) {longitude = myP.longitude;latitude = myP.latitude;}
  Point.empty(){longitude = 0;latitude = 0;}
  @override
  String toString()
  {
  ...
  Point operator -(Point v) => Point(longitude - v.longitude, latitude - v.latitude);
  double directionInAngle()//gets direction offset and returns the angle from the north pole
  {
 ...
 List<Point> polylineDecode(String str)
{
 ...
 List<Point> points = [];
 int  i = 0;
 bool flag = true;//flag for telling if current number we fetch from the string is the longitude or latitude
 double tempLongitude = 0,tempLatitude = 0;
  ...
  //  int temp = asciiValues[i] & 0x1f;<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
    ...
    if(flag == true) { tempLatitude = finalNumber; flag = false;}
    else { 
      ...
      Point newPoint = Point(double.parse((tempLongitude).toStringAsFixed(5)) ,double.parse((tempLatitude).toStringAsFixed(5)) );
       points.add(newPoint);
      flag = true;
      }
  }
...
List finalPosition(List<Point> points)//, cTime theTime, Point sunPosition)
{
  ...
  while(it.moveNext())
  {
    ...
    if(offsetOfSunAndDirection >0 && offsetOfSunAndDirection < 180 && spa.zenith! <87 && spa.zenith! > 0)
    {
      countRight+= offsetToNext.distance();
    }
    else if((offsetOfSunAndDirection <0 || offsetOfSunAndDirection > 180) && spa.zenith! <87 && spa.zenith! > 0)
    {
  ...
  if(countRight == countLeft) {
     if (countRight == 0)  {print("sit Where ever you want!");returning = 4;} 
     returning = 3;
    }
  else if(countLeft > countRight) {print("sit Right");returning = 1;}//left
  else print("sit Left");returning =  2;//right
